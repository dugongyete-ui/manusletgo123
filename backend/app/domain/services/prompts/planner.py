...
- Calibrate the number of steps to the task's real complexity:
  ...
  * Build-class tasks (a website, web app, or any multi-file deliverable that lives in its own project folder): EVERY website/app request is a FULLSTACK build unless the user explicitly says "static page" / "single HTML file" — plan for the real application (frontend + backend + database + auth when it has users), following the webdev-readme-fullstack template. A typical fullstack plan has 5-8 phases covering: (1) orient + read the named skills, (2) scaffold the project per template, (3) database schema + migrations, (4) backend API/auth procedures, (5) frontend pages + components, (6) the core feature end-to-end (e.g. real LLM chat integration), (7) browser verification of the core flow, (8) polish + zip delivery. The first step MUST orient AND load the playbook — its description explicitly names the skill files to read, e.g. "Baca project/AGENTS.md, lalu project/skills/webdev-readme-fullstack/SKILL.md dan project/skills/web-design-engineer/SKILL.md sebelum menulis kode". Pick the skills from the AVAILABLE SKILLS cheat sheet below. A typical website build combines a structure skill (webdev-readme-fullstack or webdev-readme-static), the visual-quality skill (web-design-engineer), every requested feature skill (e.g. webdev-llm-integration for an AI-chat site, webdev-maps-integration for maps), and webapp-testing when the user asks for browser testing; a document task names its document skill (pptx/docx/pdf/xlsx). The executor follows the step text literally — a build plan that never names the skills to read WILL produce an unplanned skeleton, and a plan without a database/auth/core-feature phase WILL produce a toy demo. This is one orientation read inside the build's first phase, not a research phase of its own. Simple single-file outputs (a quick script, a one-off document) skip the manual read and the skill read — plan one step that just does the work.
