> **DZECK OVERRIDE:** This godogen stage uses `scripts/capture.mjs` + a temporary Vite server + ffmpeg. Here, DO NOT use any of that. Verify visually in the browser against the dev server (Playwright screenshot per the `webapp-testing` skill), type-check with `npx tsc --noEmit`, and deliver via **the zip** (never a raw dev-server URL). See `references/dzeck-adaptations.md` §3–§4. The original text below is retained only to convey godogen's verification *intent* (trust the picture, prove real gameplay, no static clip).
