...
- project/skills/ holds 50 focused playbooks (web apps, research, data analysis, packaging…), one folder per skill with a SKILL.md. The index with load tiers is project/SKILLS.md — top level of project/, next to AGENTS.md (an identical copy sits at project/skills/SKILLS.md; both paths resolve) — build skills before any matching build, feature skills only when that feature is actually requested. Open the matching project/skills/<name>/SKILL.md before you start building — it carries the lessons that turn "it ran once" into "it actually works".
