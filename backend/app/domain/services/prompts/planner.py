...
- Calibrate the number of steps to the task's real complexity:
  ...
  * Any task that will create or modify files: the first step begins by orienting in the workspace — read project/AGENTS.md (the operating manual: where builds go, which skill matches, delivery rules), a quick look at what already exists — then build. One read, not a research phase.
