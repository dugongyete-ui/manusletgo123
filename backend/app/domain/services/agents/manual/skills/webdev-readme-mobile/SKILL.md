...
description: Dzeck webdev mobile-app (Expo) projects — the complete development guide for the Expo template that mobile-app projects are built on. Covers Expo conventions, design and branding rules, file layout, and where the backend guide lives. Read it BEFORE scaffolding a mobile app.
---
...
**This sandbox (Replit / E2B):** there is no pre-scaffolded Expo project — YOU create these files yourself with `file_write`, using this guide as the blueprint, inside `<home>/project/<app-name>/`. Use `npm` (pnpm is not installed). There is no Android emulator or Xcode here: verify with `npm test` (Vitest) plus TypeScript checks, and with `npx expo start --web` + the browser tools when the app targets web; native device builds go through the user's own EAS account — the README documents the commands. The backend guide is the `webdev-readme-mobile-backend` skill, and the database doctrine is the same as `webdev-readme-fullstack`: SQLite (drizzle-orm + better-sqlite3), no database server in this sandbox.
...
| **User Auth** | Session login (JWT), session management | User accounts |
| **Database** | SQLite + Drizzle ORM (MySQL/TiDB when the user provides one) | Cross-device data sync |
| **File Storage** | Local-disk storage (S3-shaped interface) | User-uploaded files |
...
See the webdev-readme-mobile-backend skill (`project/skills/webdev-readme-mobile-backend/SKILL.md`) for implementation details.
...
// Bundle ID format: space.dzeck.<project_name_dots>.<timestamp>
// e.g., "my-app" created at 2024-01-15 10:30:45 -> "space.dzeck.my.app.t20240115103045"
...
const bundleId =
  rawBundleId
    ...
    .join(".") || "space.dzeck.app";
// Extract timestamp from bundle ID and prefix with "dzeck" for deep link scheme
// e.g., "space.dzeck.my.app.t20240115103045" -> "dzeck20240115103045"
const timestamp = bundleId.split(".").pop()?.replace(/^t/, "") ?? "";
const schemeFromBundleId = `dzeck${timestamp}`;

const env = {
  // App branding - update these values directly (do not use env vars)
  appName: "{{project_title}}",
  appSlug: "{{project_name}}",
  // URL of the app logo - generate a logo with the image_generate tool, save it under assets/images/, and set that path here
...
```json
{
  ...
  "scripts": {
    "dev": "concurrently -k \"npm run dev:server\" \"npm run dev:metro\"",
