...
description: Dzeck webdev mobile-app (Expo) projects — backend development guide (session auth, database/Drizzle, tRPC API, env, testing, integrations). MUST read BEFORE using any backend/server/database features in a mobile-app project.
---
...
**This sandbox (Replit / E2B):** nothing here is pre-scaffolded — YOU create these files with `file_write` following this guide, inside `<home>/project/<app-name>/`. Use `npm` (pnpm is not installed). This sandbox has NO database server: the required database is SQLite (drizzle-orm + better-sqlite3 — file-based, zero config); the MySQL listings below are the pattern reference — same table shapes, `sqlite-core` imports. Nothing is platform-injected: session auth is JWT (auth section below), LLM keys are the user's own (`webdev-llm-integration` skill), storage is local disk (`webdev-file-storage` skill), and migrations are applied with `shell_exec` (better-sqlite3 or the `sqlite3` CLI), not a managed SQL tool.
...
| User accounts / login | Yes | Yes | Session auth (JWT) |
...
The template uses **JWT session auth** — the backend verifies credentials (bcrypt) and issues a signed session token. It works differently on native and web:
...
interface User {
  id: number;
  openId: string;        // login subject (email / username / provider id)
  name: string | null;
  email: string | null;
  loginMethod: string;
  role: "user" | "admin";
...
2. `WebBrowser.openAuthSessionAsync()` opens the app's own login screen (or an OAuth provider the user chose)
...
2. Browser redirects to the app's login screen
3. User authenticates
...
After editing the schema, run `npx drizzle-kit generate` to produce migration SQL, then read the generated `.sql` file and apply it via `shell_exec` (better-sqlite3 or the `sqlite3` CLI). Keep the TypeScript schema and actual database in sync.
...
Implement the LLM helper in `server/_core/llm.ts` following this shape — it calls an OpenAI-compatible endpoint with the key from `.env` (the user provides `LLM_API_KEY` / `LLM_API_URL`); see the `webdev-llm-integration` skill.
...
- LLM calls bill the user's own provider account — surface provider errors honestly.
...
- For image-based gen AI workflows, local `file://` and blob URLs don't work. Upload to the app's storage (`/uploads/`) first, then pass the public URL to `invokeLLM()`.
...
- The returned `url` (e.g., `/uploads/...`) is served by the Express static route — no manual URL signing needed.
...
When you need external data, call the provider's public API directly from server code with the user's own key in `.env` — there is no built-in API hub in this environment.

---

## Owner Notifications

Implement a `notifyOwner({ title, content })` helper (`server/_core/notification.ts`) and a protected tRPC mutation at `trpc.system.notifyOwner` following this shape. Use it whenever backend logic needs to push an operational update to the Dzeck project owner—common triggers are new form submissions, survey feedback, or workflow results. The channel is whatever the owner configures in `.env` — an email (SMTP) or a webhook URL (see the `webdev-owner-notifications` skill).
...
| `JWT_SECRET` | Session signing secret — generate one, never hardcode |
| `OWNER_NAME` | Owner's display name |
| `LLM_API_URL` | OpenAI-compatible endpoint for LLM features (user-provided) |
| `LLM_API_KEY` | Provider API key for LLM features (user-provided) |
...
export const users = mysqlTable("users", {
   ...
   */
  id: int("id").autoincrement().primaryKey(),
  /** Login subject (openId) — email, username, or provider id. Unique per user. */
  openId: varchar("openId", { length: 64 }).notNull().unique(),
  name: text("name"),
  email: varchar("email", { length: 320 }),
...
// Storage helpers for Dzeck-built apps — local disk under data/uploads/
// (the interface stays S3-shaped so a real bucket can drop in later).
// Downloads return /uploads/{key} paths served by the Express static route.
...
export async function storageGetSignedUrl(relKey: string): Promise<string> {
  // Local-disk storage needs no signing — the static route serves the file.
...
The sections above cover the basics; each integration also has its own skill doc with the full contract — read it BEFORE implementing a feature that depends on one:

| Integration | Skill doc | When to use |
|---|---|---|
| LLM / AI chat | `project/skills/webdev-llm-integration/SKILL.md` | AI features, chat completions, structured JSON responses, model listing/selection, streaming, thinking/reasoning |
| Voice transcription | `project/skills/webdev-voice-transcription/SKILL.md` | Speech-to-text via Whisper API |
| Image generation | `project/skills/webdev-image-generation/SKILL.md` | AI image creation or editing, model listing/selection |
| File storage (S3) | `project/skills/webdev-file-storage/SKILL.md` | Uploading/serving user files, images, documents |
| Owner notifications | `project/skills/webdev-owner-notifications/SKILL.md` | Push alerts to the project owner |
| Periodic updates / cron | `project/skills/webdev-periodic-updates/SKILL.md` | Scheduled work — recurring jobs, end-user-scheduled cron, periodic notifications. MUST read before any scheduled-work code |
