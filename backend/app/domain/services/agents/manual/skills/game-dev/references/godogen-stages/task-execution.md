...
> **DZECK OVERRIDE:** Same inner loop, but the project is a React + Vite app. Edit under `client/src/game/**`, rely on Vite HMR, verify in the browser (not `capture.mjs`), type-check with `npx tsc --noEmit`, inspect the dev-server session via `shell_view`. See `references/dzeck-adaptations.md` §3.
