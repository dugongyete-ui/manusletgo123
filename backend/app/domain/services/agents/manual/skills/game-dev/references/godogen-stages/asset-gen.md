...
> **DZECK OVERRIDE:** Do NOT call the paid `asset_gen.py` / Gemini / Grok / Tripo3D CLIs described below. Here, generate all images with the `image_generate` tool, then save them under `client/public/assets/game/` and use the `/assets/game/...` path as Babylon textures. For 3D, default to procedural meshes + generated textures; only use GLB models if the user supplies a Tripo3D key. See `references/dzeck-adaptations.md` §2. The text below conveys godogen's asset *intent* (model strengths, sizing, transparency) for reference only.
