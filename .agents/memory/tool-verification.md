...
**Script:** `/home/z/my-project/scripts/verify_all_tools.py --provider replit|e2b|both`
...
- Result: **57/57 PASS on Replit, 57/57 PASS on E2B** — reports in `/home/z/my-project/download/tool-verification-<provider>.json`.
...
- `SANDBOX_PROVIDER=replit` is the current setting; E2B (`e2b:*` microVMs via HybridSandboxFactory, `auto`/`e2b`/`replit`) is fully functional with a valid key — verified 2026-08-30, quota OK.
