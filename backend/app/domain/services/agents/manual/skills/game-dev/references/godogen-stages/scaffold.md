...
> **DZECK OVERRIDE:** Do NOT create godogen's standalone Vite/index.html/main.ts shell. The host is a React + Vite + Tailwind project scaffolded per the `webdev-readme-static` skill: `npm install @babylonjs/core`, then mount the full-screen `GameCanvas.tsx` (see `templates/GameCanvas.tsx`) as the `/` route. Gameplay still lives in framework-agnostic TS under `client/src/game/**`, matching this stage's intent. See `references/dzeck-adaptations.md` §1.
