...
> **DZECK OVERRIDE:** Ignore the dollar-budget gating below. Here the art step is always-on via the `image_generate` tool. Use this stage only to decide *what* assets the game needs (type, size, role); generate them per `references/dzeck-adaptations.md` §2.
