...
description: Dzeck webdev fullstack (web-db-user) & mobile-app (Expo) projects — scheduled work reference (Heartbeat) for recurring jobs, daily/hourly tasks, end-user-scheduled cron, periodic notifications. Read FIRST before planning or coding anything that runs on a schedule.
---
...
Forbidden on hosts that terminate idle instances: `setInterval`, `node-cron`, or any in-process timer — Cloud Run terminates idle instances and free-tier Repls sleep, so in-process timers will not survive there. If the user self-hosts a long-running process (VPS, always-on deployment), an in-process driver over a `scheduled_jobs` table is acceptable — pick per the user's hosting plan.
...
- **Heartbeat (HTTP cron).** The scheduler POSTs directly to `/api/scheduled/*` on this site. Your handler runs and returns. No agent spawned — if the handler needs an LLM call, do it inline via the site's own LLM SDK (`server/_core/llm.ts`).
- **AGENT cron.** Platform spawns a fresh, isolated Dzeck session that runs the prompt you wrote at create time. The agent has the full Dzeck toolchain — browser, file system, shell, image gen, deep research — but **no** session history, DB, source code, or credentials beyond the two env vars `$SCHEDULED_TASK_ENDPOINT_BASE` / `$SCHEDULED_TASK_COOKIE`. If your prompt tells it to, it `curl`s back to `/api/scheduled/*` on this site at the end.
...
1. Callback path **MUST** start with `/api/scheduled/`. The scheduler rejects everything else.
...
3. The callback only fires once the app is actually RUNNING — the user starts it (`npm start`) or deploys it. In dev, trigger it manually once with `curl -X POST` (with the cron token) to verify the handler before shipping. For handler changes, save a checkpoint (commit) first, then ask the user to restart or deploy before any schedule actions take place.
4. Wrap handler logic in try/catch and JSON-encode the error on 500 — the operator should see the failure verbatim in logs and in the scheduler's response.
...
`sessionToken` MUST be the decoded `app_session_id` cookie value, not the raw `Cookie` header — the scheduler attributes the cron to the requesting end-user via this token. Apply your normal ownership check on the business row before scheduling.
...
if (!campaign) return res.json({ ok: true, skipped: "orphan" }); // 2xx so the scheduler stops retrying
...
After deploy, end-users create their own crons via your tRPC mutation. As the project owner, you can also inspect / pause / resume / view logs for any end-user's cron from the sandbox terminal — `dzeck-heartbeat list --user-id u_xxx` and friends; see §5b. End-users can never see another user's cron through your tRPC SDK.

---

## 4. Variants — when the trigger isn't an end-user

Same callback handler, same `/api/scheduled/*` URL, same `user.isCron` check, **same deployment-managed cron lifecycle** (cron persistence and triggering live entirely with the deployment target, independent of any sandbox session). Only the **creator** changes.
...
The created cron lives with the deployment target, not in this sandbox: it survives sandbox teardown and keeps firing as long as the deployed site is reachable. Any future session can `dzeck-heartbeat list` / `update` / `pause` / `delete` it — the cron is bound to the project owner, not to the build session that created it.

Persist the returned `task_uid` somewhere durable (admin DB row / config) if you'll need to update/delete it later — `dzeck-heartbeat list` can also recover it.

### 4b. AGENT cron — when the trigger needs agentic capabilities

Created by the operator or platform at create time (not from site code). Each trigger spawns a **fresh, isolated** Dzeck agent — no session history, no source/DB/credentials, no skills. The only knowledge transfer is whatever you write into the cron prompt.
...
### 4c. Owner UI on the hosting platform (NOT something you build)

The hosting platform's dashboard (Replit, cron-job.org, a VPS panel) surfaces ALL crons in the project (both end-user-driven and agent-driven), with execution history, pause/resume, edit, Run Now, and Investigate. Owners can't *create* crons there — only via §3 / §4a / §4b. Mention this to the user when they ask "how do I see/manage all my crons".
...
In this workspace nothing auto-scaffolds `heartbeat.ts` — you implement this contract yourself (a `scheduled_jobs` table + the four functions), with cron-caller auth wired per §5c.

### 5b. Sandbox CLI — `dzeck-heartbeat`

In-sandbox tool that hits the same backend as the SDK in §5a but with project owner identity (no end-user cookie). **Not pre-installed in this workspace** — after delivery, provide the same command surface as a small `node server/cli/heartbeat.js` implementing §5a, or manage the `scheduled_jobs` rows directly.

The table below is just an index — `dzeck-heartbeat <cmd> --help` is canonical for full flag lists and examples.
...
> Applies whenever `server/_core/heartbeat.ts` is missing — in this workspace that is every project (nothing is auto-scaffolded): write these patches in while building `sdk.ts` and the types. If `server/_core/sdk.ts` already contains `CRON_OPEN_ID_PREFIX`, skip this section.
...
**`server/_core/types/dzeckTypes.ts`** — add the cron-only `taskUid` field to `GetUserInfoWithJwtResponse` (everything else stays as-is):
...
function buildCronUser(userInfo: GetUserInfoWithJwtResponse): AuthenticatedUser {
  ...
  return {
    ...
    name: userInfo.name || "Dzeck Scheduled Task",
    email: null,
    loginMethod: null,
    role: "user",
    createdAt: now,
...
These two files are server-side; the dev server must restart to pick them up. Save a checkpoint (commit), then restart before testing the new tRPC handlers.
