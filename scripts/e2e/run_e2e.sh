...
#   E2E_LOGIN      EMAIL:PASSWORD — login as this account (fallback: register)
#   E2E_UPLOAD     space-separated list of files to upload & attach
#   E2E_POLL_AFTER seconds to poll session status after stream timeout
#                  (long tasks keep running server-side; exit 4 = still running,
#                   re-run with E2E_ATTACH_ONLY=1 E2E_NO_SEND=1 to continue)
#   E2E_KEEP_ALIVE=1     skip the final cleanup (stack stays up for the next call)
#   E2E_ATTACH_ONLY=1    don't start the stack, just run the driver against it
...
LOGIN="${E2E_LOGIN:-}"
UPLOAD="${E2E_UPLOAD:-}"
POLL_AFTER="${E2E_POLL_AFTER:-0}"
KEEP_ALIVE="${E2E_KEEP_ALIVE:-0}"
ATTACH_ONLY="${E2E_ATTACH_ONLY:-0}"
...
" 2>/dev/null; then
  ...
  echo "== attach-only mode: reusing running stack =="
  curl -s --max-time 3 http://localhost:8000/health 2>/dev/null | grep -q '"ready":true' \
    && echo "backend alive" || echo "!! backend NOT alive"
  curl -s --max-time 3 http://localhost:8222/json/version 2>/dev/null | grep -q "Browser" \
    && echo "chrome CDP alive" || echo "!! chrome CDP NOT alive"
fi
...
if [ -n "$LOGIN" ]; then DRIVER_ARGS="$DRIVER_ARGS --login $LOGIN"; fi
for f in $UPLOAD; do DRIVER_ARGS="$DRIVER_ARGS --upload $f"; done
if [ "$POLL_AFTER" != "0" ]; then DRIVER_ARGS="$DRIVER_ARGS --poll-after $POLL_AFTER"; fi
if [ -n "$MSG" ]; then
...
if [ "$KEEP_ALIVE" = "1" ] && [ "$DRIVER_EXIT" != "0" ] && [ "$DRIVER_EXIT" != "3" ]; then
  echo "== keep-alive: stack left running (task continues server-side) =="
  exit $DRIVER_EXIT
fi
...
tail -25 /tmp/backend_e2e.log 2>/dev/null
exit $DRIVER_EXIT
