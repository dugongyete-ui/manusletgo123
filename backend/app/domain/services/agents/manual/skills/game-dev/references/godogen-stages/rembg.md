...
> **DZECK OVERRIDE:** Instead of this background-removal CLI, prompt the `image_generate` tool directly for a clean transparent/cutout background. Only fall back to programmatic removal if generation cannot produce clean alpha. See `references/dzeck-adaptations.md` §2.
